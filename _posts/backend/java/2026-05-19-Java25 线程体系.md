...
* 阻塞 (Blocking) ： 当线程发起一个操作（如读取文件、网络请求、获取锁）时，如果条件不满足，该线程会被操作系统**挂起（进入 BLOCKED/WAITING 状态）**，暂停执行并释放 CPU 资源，直到条件满足后被唤醒。
  ...
  *注：此段堵塞代码与上面同步代码的区别：同步代码可能因为堵塞而耗时（线程处于挂起），也可能因为计算复杂而耗时（线程处于执行中）如自旋锁，而此段堵塞代码线程一定处于挂起状态。*

* 非阻塞 (Non-blocking)：当线程发起一个操作时，无论结果如何，它都会**立即返回**。如果条件不满足，它会马上得到一个错误码或空结果，而**不会被挂起**，可以继续去处理其他事情。通常会配合轮询（不断询问）或 I/O 多路复用（让内核通知）来使用。（*非堵塞指的是线程在操作系统层面没有被挂起，没有让出CPU*）
